...
package object barneshut {
  ...
  case class Fork(
    ...
    val q = List(nw, ne, sw, se)
    val centerX: Float = (q.map(_.centerX).min + q.map(_.centerX).max) / 2f
    val centerY: Float = (q.map(_.centerY).min + q.map(_.centerY).max) / 2f
    val size: Float = nw.size * 2
    val mass: Float = q.foldLeft(0f)(_ + _.mass)
    val massX: Float = {
      if (mass == 0) centerX
      else q.foldLeft(0f) { case (sum, quad) => sum + quad.mass * quad.massX / mass }
    }
    val massY: Float = {
      if (mass == 0) centerY
      else q.foldLeft(0f) { case (sum, quad) => sum + quad.mass * quad.massY / mass }
    }
  ...
  extends Quad {
    val (mass, massX, massY) = (
      ...
      bodies.foldLeft(0f) { case (sum, b) => sum + b.mass * b.x } / bodies.foldLeft(0f)(_ + _.mass) : Float,
    ...
    def insert(b: Body): Quad = {
      ...
      if (size <= minimumSize) Leaf(centerX, centerY, size, b +: bodies)
        ...
        val quadSize = size / 4
        val halfSize = size / 2
        val wx: Float = centerX - quadSize
        val ex: Float = centerX + quadSize
        val ny: Float = centerY - quadSize
        val sy: Float = centerY + quadSize
        val fork = Fork(
          Empty(wx, ny, halfSize),
          Empty(ex, ny, halfSize),
          Empty(wx, sy, halfSize),
          Empty(ex, sy, halfSize))
