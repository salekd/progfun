...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    val occ = occurrences.map(x => (for (i <- 1 to x._2) yield (x._1, i)).toList)
    occ.foldRight(List[Occurrences](Nil))((x, y) => y ++ (for(i <- x; j <- y) yield (i :: j)))
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val (common, other) = x.partition(a => y.exists(b => a._1 == b._1))
    val sub = for ((a, b) <- common.zip(y) if a._2 != b._2) yield (a._1, a._2 - b._2)
    (other ++ sub).sorted
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def f(o: Occurrences): List[Sentence] = {
      if (o.isEmpty) List(Nil)
      else {
        val c = combinations(o)
        for {
          i <- c if dictionaryByOccurrences.keySet(i)
