...
object TimeUsage {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primaryStartWith = List("t01", "t03", "t11", "t1801", "t1803")
    val workStartWith = List("t05", "t1805")
    val otherStartWith = List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")

    (
      columnNames.filter( name => primaryStartWith.map(start => name.startsWith(start)).foldLeft(false)(_ | _) )
        .map(name => new Column(name)),
      columnNames.filter( name => workStartWith.map(start => name.startsWith(start)).foldLeft(false)(_ | _) )
        .map(name => new Column(name)),
      columnNames
        .filter( name => otherStartWith.map(start => name.startsWith(start)).foldLeft(false)(_ | _) )
        .filter( name => !primaryStartWith.union(workStartWith).map(start => name.startsWith(start)).foldLeft(false)(_ | _) )
        .map(name => new Column(name))
      )
  }
  ...
  def timeUsageSummaryTyped(timeUsageSummaryDf: DataFrame): Dataset[TimeUsageRow] =
    timeUsageSummaryDf.map(r => TimeUsageRow(
      r.getAs("working"), r.getAs("sex"), r.getAs("age"), r.getAs("primaryNeeds"), r.getAs("work"), r.getAs("other"))
    )
